...
where directors_id = '5';
update directors
set first_name ='Sidhant'
where directors_id = '5';
update directors
...
select first_name
from directors
where directors_id = '4' and nationality = 'American'
...
--Create movie table (with foreign key)
...
(
    ...
    directors_id      int references directors (directors_id)--foreign key.
);

INSERT INTO movies (movie_name, movie_length, movie_language, release_date, age_certification, directors_id)
...
(
    ...
    movie_id              int references movies (movie_id),--foreign key.
    domestic_revenue      numeric(6, 2),
    international_revenue numeric(6, 2)
);
INSERT INTO movieRevenue (revenue_id, movie_id, domestic_revenue, international_revenue)
...
INSERT INTO moviesActors (actor_id, movie_id)
VALUES ('1', '52'),
       ('2', '50'),
       ('3', '23'),
       ('4', '26'),
...
select movie_length, movie_name
from movies
...
from movies
where movie_language in ('English', 'Spanish', 'Korean');
...
select directors_id as id
from directors;
select directors_id
from directors;

select international_revenue
from movieRevenue
where international_revenue notnull
...
select max(domestic_revenue)
from movieRevenue;
select min(domestic_revenue)
from movieRevenue;
...
select movie_language, age_certification, sum(movie_length)
from movies
group by movie_language, age_certification
order by movie_language, age_certification asc;
...
select directors.directors_id, directors.first_name, directors.last_name, movies.release_date
from directors
         join movies on directors.directors_id = movies.directors_id
order by directors_id;


select m.movie_name, m.release_date, mR.international_revenue
from movieRevenue mR
         join movies m on m.movie_id = mR.movie_id
where m.movie_language = 'English';

select d.first_name, d.last_name, mo.movie_name, d.nationality
from movies mo
         left join directors d on mo.directors_id = d.directors_id
...
from directors d
         left join movies mo on d.directors_id = mo.directors_id
group by d.first_name, d.last_name
order by count(mo.movie_id);
...
from actors ac
         join moviesActors mA on ma.actor_id = ac.actor_id
         join movies mo on mo.movie_id = mA.movie_id
         join directors d on d.directors_id = mo.directors_id
where d.first_name = 'Wes'
  and d.last_name = 'Anderson';



select d.first_name, d.last_name, sum(mR.domestic_revenue) as total_domestic_revenue
from directors d
         join movies mo on mo.directors_id = d.directors_id
         join movieRevenue mR on mR.movie_id = mo.movie_id
where mR.domestic_revenue is not null
group by d.first_name, d.last_name
order by total_domestic_revenue desc
limit 1;
...
where movie_id in
      (select movie_id
       from movieRevenue
       where domestic_revenue >
             (select avg(domestic_revenue)
              from movieRevenue));


select ac.first_name, ac.last_name, ac.date_of_birth,ac.gender
from actors ac
where date_of_birth =
      (select min(date_of_birth)
       from actors ac2
       where ac.gender = ac2.gender);

select mo1.movie_name, mo1.movie_length, mo1.age_certification
from movies mo1
where movie_length >
      (select avg(mo2.movie_length)
       from movies mo2
       where mo1.age_certification = mo2.age_certification)
order by age_certification;


select d.first_name, d.last_name, upper(mo.movie_name) as movies
from movies mo
         join directors d on mo.directors_id = d.directors_id;

select initcap(ac.first_name)as first_name, initcap(ac.last_name)as last_name
from actors ac
         join moviesActors mA on ac.actor_id = mA.actor_id
         join movies m on mA.movie_id = m.movie_id
where movie_language = 'Korean'
   or movie_language = 'Chinese';

select reverse(first_name) as first_name, reverse(last_name) as last_name, "left"(upper(nationality), 3) as Nationality
from directors;

select concat_ws(' ', "left"(first_name, 1), "left"(last_name, 1)) as initaials
from directors;
